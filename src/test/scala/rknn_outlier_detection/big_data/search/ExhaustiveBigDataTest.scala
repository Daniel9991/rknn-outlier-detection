...
class ExhaustiveBigDataTest extends AnyFunSuite {
    ...
    val sc = new SparkContext(new SparkConf().setMaster("local[2]").setAppName("Sparking2"))
...
//        .getOrCreate();
    ...
    test("Empty RDD"){
        ...
        val result = searchStrategy.findKNeighbors(testingData, 1, DistanceFunctions.euclidean, sc)
