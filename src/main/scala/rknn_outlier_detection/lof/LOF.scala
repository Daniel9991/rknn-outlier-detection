...
package rknn_outlier_detection.lof
...
class LOF(
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val indexedPointsRDD = dataset.select("id", "features").rdd.map {
      case Row(id: Long, vec: Vector) => (id, vec)
    }.persist()
...
//    session.createDataFrame(finalRDD, schema)
    ...
    val schema = StructType(Array(
      ...
      StructField(LOF.lof, DataTypes.DoubleType, false)
    ))

    val resultDF = session.createDataFrame(finalRDD).toDF("id", "lof")
...
object LOF {
  ...
  val supportedDistTypes = Array(euclidean)
